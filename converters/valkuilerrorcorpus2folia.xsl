...
<xsl:template match="dcoi:w">
 <w xmlns="http://ilk.uvt.nl/folia">
  ...
  <xsl:choose>
  <xsl:when test="@original and @original != text() and (not(@correction) or not(contains(@original,' ') or contains(text(),' ') or contains(@correction, ',') or contains(@correction,'split')  or contains(@correction,'run_on') ))">
  	<correction>
  	  	<xsl:if test="@xml:id">
  		...
  		<xsl:choose>  		
	  		<xsl:when test="@correction">
	   			<xsl:attribute name="class"><xsl:value-of select="@correction" /></xsl:attribute>
	  		</xsl:when>
			<xsl:otherwise>
