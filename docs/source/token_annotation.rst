...
:**Element**: ``<w>``
:API Class: ``Word`` (`FoLiApy API Reference <https://foliapy.readthedocs.io/en/latest/_autosummary/folia.main.Word.html>`_)
:Required Attributes: 
...
.. note::
    ...
    This element carries an extra and optional ``space`` attribute with value ``yes`` (default), or ``no``, indicating
    whether a space follows between this token and the next one. This attribute is used to reconstruct the untokenised
    text.
