...
\emph{January 2nd, 2014 \small{(published)} -- March 22nd, 2015 \small{(last revision)}} \\[0.3cm] 
...
\status{final since v0.3 (older versions are equal but lack declarations), larger quotes allowing paragraphs and division since v0.11.3}{pynlpl, libfolia}

FoLiA supports quotes, using the \texttt{quote} element, to indicate that the structural elements within are what
...
Below you see an example of a glossary entry, the sense set used comes from WordNet. The other sets are ficticious.
...
ficticious in our examples, giving the user flexibility. Definitions are for instance suited for dictionaries:
