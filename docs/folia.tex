...
Like token annotations, subannotation layers can be included in \texttt{alt} elements.
...
Optionally, \texttt{X} itself can also be associated with a class, the class in this case would implicitly or explicitly represent all sub-parts, or be the basis upon which redefinement is made.

Elements such as the ficticious \texttt{Y} can be implicitly associated with a specific subset.
...
FoLiA implicitly relates the synset attribute here to the ``synset'' subset of whatever set you defined. Note that any such implicit subset relations need to be defined by the FoLiA standard itself, you can't make up such attributes yourself!
