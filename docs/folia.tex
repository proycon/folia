...
{\bf \small version 0.10.1 -- Revision 3.4} \\[0.5cm]
{ \Large \bfseries Documentation}\\[0.5cm]
{\large \emph{Maarten van Gompel}}\\[0.5cm]
\HRule \\[1.0cm]

\emph{January 2nd, 2014} \\[0.5cm] 
...
\status{since FoLiA 0.11.0}{pynlpl}
...
are made using a specific tag: \texttt{noteref}
...
A reference can then be made as such, this acts like a structure element but
has a higher order annotation function:
...
Another example in tokenised data:
...
<s>
  <t>We demonstrated this earlier.</t>
  <noteref id="mynote"><t>1</t></noteref>
</s>
\end{lstlisting}


Being a structure element, the note reference itself may carry an ID as well.
Note that the ID attribute without the xml namespace always indicates a reference in
throughout FoLiA:

\begin{lstlisting}[language=xml]
<s><t>We demonstrated this earlier.</t></s>
<noteref xml:id="myreference" id="mynote" />
\end{lstlisting}
...
\status{proposed in v0.10.1 for v0.11}{not implemented yet}
...
\begin{itemize}
  ...
  \item Make sure all external files describe the same version of the document
