...
per file, any other defined sets must be subsets of the primary set. Moreover,
FoLiA Set Definitions must make use of the FoLiA Set Definition Schema, an RDF
schema which defines the model used. The schema can be obtained from
\url{http://folia.science.ru.nl/setdefinition} in Turtle format. This URI is
also the RDF namespace. Some familiarity with RDF and Turtle is recommended for this
chapter, but it is also still possible to work with the XML legacy format,
which is a bit more concise and simple, and automatically convert it to Turtle format.

Your own set definitions should each have their own RDF namespace, which in
...
<#your-set-id> 
	a fsd:Set ;
	fsd:id	"your-set-id" ;
	fsd:label	"Human readable label for your set" ;
	fsd:open	false .
\end{lstlisting}

The first two lines state that \texttt{http://your/namespace/\#your-set-id} is
\emph{a}\footnote{the \emph{a} in Turtle is shorthand for rdf:type} FoLiA Set.
The other define specific FoLiA Set Definition (fsd) properties.

A set can be either open or closed (default), an open set allows any classes,
even if they are not defined. This can be used for open vocabularies.

In our legacy XML format, this would look like:
...
(the FoLiA namespace) and has nothing to do with the aforementioned RDF
namespace.

\subsection{Classes}

A set consists of classes. Consider a simple part-of-speech set with three classes. First we define the set:

\begin{lstlisting}
<#simplepos> 
	a fsd:Set ;
	fsd:id	"simplepos" ;
	fsd:label "A simple part of speech set" .
\end{lstlisting}
...
The \texttt{ID} of the class (mandatory) determines a value the \texttt{class} attribute
...
You may want to associate classes, or even sets themselves, with other linked
data collections or data category registries. This is done using the \texttt{fsd:conceptLink} relation, assuming the target is an RDF resource. Example:
...
<#N> 
	...
	fsd:label "Noun" ;
	fsd:memberOf <#simplepos> ;
	fsd:conceptLink <http://wiktionary.dbpedia.org/terms/#Noun> ;
	fsd:infoLink "https://en.wikipedia.org/wiki/Noun" .
\end{lstlisting}

Multiple concept links are allowed. If you want to link to something that is
not a formal RDF resource, but for instance a website providing information,
then use the \texttt{fsd:infoLink} relation instead:
...
In Turtle syntax, this is more verbose as the hierarchy has to be modelled explicitly using the \texttt{rdf:parentClass} property, as shown in the following excerpt:

\begin{lstlisting}
<#namedentities> 
	a fsd:Set ;
	fsd:id	"namedentities" .

<#loc> 
	a fsd:Class ;
	fsd:id	"loc" ;
	fsd:label "Location" ;
	fsd:memberOf <#namedentities> .
...
In Turtle syntax, these are
defined just like the primary set, but take an extra \texttt{rdf:subsetOf}
property that explicitly refers to the parent set:
...
ID in other subsets as well. The \texttt{fsd:id} property does not need to
