...
{
    \footnotesize{(Note: The given sets above are just examples and do not exist, you
            should either create and use a set of your own, or pick a suitable existing set, if available)} \\\hline
    \end{tabular}
\end{center}}

\newenvironment{setdefinitions}{
\begin{center}
    \begin{tabular}[h!]{|p{0.9\textwidth}|}
    \hline
    {\bf \textsf{Available sets}}\\\hline}
{
    \footnotesize{(Note: If no usable set exists yet, simply create and use
    your own)} \\\hline
    \end{tabular}
\end{center}}
...
{\bf \small version 1.5.0 -- Revision 8.1} \\[0.5cm]
{ \Large \bfseries Documentation}\\[0.5cm]
{\large \emph{Maarten van Gompel}}\\[0.5cm]
\HRule \\[1.0cm]

\emph{January 2nd, 2014 \small{(published)} -- September 25th, 2017 \small{(last revision)}} \\[0.3cm]
...
FoLiA is a Format for Linguistic Annotation. It is an XML-based\cite{XML}
annotation format, suitable for the representation of linguistically annotated
language resources. FoLiA's intended use is as a format for storing and/or
exchanging language resources, including corpora. Our aim is to introduce a
single rich format that can accommodate a wide variety of linguistic annotation
types through a single generalised paradigm. We do not commit to any label set,
language or linguistic theory.  This is always left to the developer of the
language resource, and provides maximum flexibility.

XML is an inherently hierarchic format. FoLiA does justice to this by
utilising a hierarchic, inline, setup where possible.
...
Inspiration for parts of FoLiA has been drawn from study of similar
initiatives, such as the Kyoto Annotation Format \cite{KYOTO} and its successor NAF
\cite{NAF}, as well as the TCF format \cite{TCF} and the PAULA format
\cite{PAULA}.
...
\item Agnostic with regard to metadata. External metadata schemes such as CMDI
    \cite{CMDI} are recommended. A limited native metadata format is available
    as well.
\item Associate descriptions or comments with any annotation.
\item Link to other internal annotations or to external resources from within
    any annotation.
\end{itemize}
...
\begin{itemize}
\item Attribution, statement annotation
\item Event annotation
\item Co-reference Annotation
\item Chunking/Shallow parses
\item Dependency Relations
\item Domain Annotation
\item Language Annotation
\item Lemmatisation
\item Named Entity Annotation, Multi-word expressions, Time expressions
\item Morphological Analysis
\item Observations
\item Part-of-speech annotation
\item Phonetic Analysis
\item Semantic Role Labelling
\item Semantic sense annotation
\item Sentiment Analysis / Subjectivity Annotation
\item Spelling corrections on both a tokenised as well as an untokenised level
\item Syntactic Constituency Parses
\item Time Segmentation
\end{itemize}

FoLiA support is incorporated directly into the following software:

\begin{itemize}
\item ucto\footnote{\url{https://languagemachines.github.io/ucto}} - A tokeniser which can directly output FoLiA XML
\item Frog\footnote{\url{https://languagemachines.github.io/frog}}- A PoS-tagger/lemmatiser/parser suite
\item FLAT\footnote{\url{https://github.com/proycon/FLAT}} - A web-based annotation environment for FoLiA
\item Gecco\footnote{\url{https://github.com/proycon/gecco}} - Spelling correction software
\item CLAM\footnote{\url{https://languagemachines.github.io/ucto}}- Computational Linguistics Application Mediator, will eventually have viewers for the FoLiA format.
\item PyNLPl\footnote{\url{https://github.com/proycon/pynlpl}}- Python Natural Language Processing Library, comes with a library for parsing FoLiA
\item libfolia\footnote{\url{https://github.com/LanguageMachines/libfolia}}- C++ library for parsing FoLiA
\end{itemize}
...
FoLiA's origins are derived from the D-Coi format\cite{DCOI} developed as part
of the D-Coi project by project partner at Polderland Language and Speech
Technologies B.V. The D-Coi format was designed for use by the D-Coi corpus, as
well as by its successor, the SoNaR corpus \cite{Oostdijk+08}. It posits to be
loosely based on a minimal subset of TEI\cite{TEI}.

Though being rooted in the D-Coi format, the D-Coi format is very limited.
FoLiA instead introduces a rich generalised framework for linguistic
annotation. FoLiA development started at the ILK research group, Tilburg
University, and is continued at the Centre of Language and Speech Technology of
Radboud University Nijmegen. It is being adopted in multiple projects,
primarily in the Dutch and Flemish Natural Language Processing community.

Because of the introduction of a new and much broader paradigm, FoLiA is
\emph{not} backwards-compatible with D-Coi.
...
to the XML standard, that is, it is a non-colonized name (NCName) that starts
with either a letter or an underscore and contains none other than letters,
digits, underscores, hyphens and periods. A well proven convention for IDs is
of a cumulative nature, in which you append the element name, a period, and a
sequence number, to the identifier of a parent element higher in the hierarchy.
Identifiers are always encoded in the \texttt{xml:id} attribute.
...
Most annotation elements take a \texttt{set} attribute, the value of which
...
particular class from the set, for a particular annotation instance.
...
generic FoLiA attributes are available for \emph{all} structure annotation elements in
a speech context:
...
A \emph{set definition} specifies exactly what classes are allowed in the set. It for example
...
Set definitions can be stored in several formats (see
chapter~\ref{chapter:setdefinitions}), the format may be indicated by the
\texttt{format} attribute on each declaration, its value should be a MIME type.
...
If only one set is declared, then in the document itself you are allowed to skip the set attribute on these specific
annotation elements. The declared set will automatically be the default. This is common practice as usually there is
only one set per annotation type. In other case, annotations explicitly have a \texttt{set} attribute that corresponds
to one of the declared sets.
...
An alias mechanism is also available when referring to sets by long URLs gets cumbersome. An alias can be declared as
shown in the following example:

\begin{lstlisting}[language=xml]
      <pos-annotation set="http://ilk.uvt.nl/folia/sets/CGN"
        annotator="Frog" annotatortype="auto" alias="cgn" />
      <pos-annotation set="http://ilk.uvt.nl/folia/sets/brown" alias="brown" />
\end{lstlisting}

Annotations may then use the alias instead of the full set URL in their \texttt{set} attribute. An alias has to be
unique for a given annotation type.
...
respectively when classes are assigned.
...
\begin{lstlisting}[language=xml]
<annotations>
 <division-annotation set="http://url/to/your/set"
</annotations>
\end{lstlisting}
\end{declaration}

\begin{setdefinitions}
\begin{itemize}
    \item
        \url{https://raw.githubusercontent.com/proycon/folia/master/setdefinitions/divisions.foliaset.xml}
        -- Generic set definition for divisions, defines the classes:
        \emph{document, chapter, section, subsection, abstract, introduction,
...
\begin{lstlisting}[language=xml]
<annotations>
 <gap-annotation set="http://path/to/your/set"
</annotations>
\end{lstlisting}
\end{declaration}
...
\begin{itemize}
    \item
        \url{https://raw.githubusercontent.com/proycon/folia/master/setdefinitions/gaps.foliaset.xml}
        -- Generic set definition for gaps, defines the classes:
        \emph{frontmatter, backmatter, illegible, inaudible, code, verbatim,
            other-language, cancelled}
\end{itemize}
\end{setdefinitions}
...
\begin{lstlisting}[language=xml]
<annotations>
 <event-annotation set="http://path/to/your/set" />
</annotations>
\end{lstlisting}
\end{declaration}
...
Tables, rows and cells can all be assigned classes, in which case a declaration
is mandatory:
...
use \texttt{desc} or \texttt{comment} instead. Notes themselves can contain all the
...
\begin{lstlisting}[language=xml]
<annotations>
 <note-annotation set="http://path/to/your/set" />
</annotations>
\end{lstlisting}
\end{declaration}
...
\begin{itemize}
    \item
        \url{https://raw.githubusercontent.com/proycon/folia/master/setdefinitions/notes.foliaset.xml}
        -- Generic set definition for notes, defines the classes:
        \emph{attention, caution, danger, error, hint, note, footnote, tip, warning, important, admonition}
\end{itemize}
\end{setdefinitions}
...
structure element, the references are explicitly present in the text, but with
an extra higher-order annotation function:
...
FoLiA has a set of structure elements that can be used to represent
...
further attributes are optional and are used to set defaults.
...
\begin{itemize}
    \item
        \url{http://ilk.uvt.nl/folia/sets/frog-mbpos-cgn} -- Defines the CGN tagset for Dutch, as used by the software \emph{Frog}\footnote{https://languagemachines.github.io/frog}.
    \item
        \url{https://raw.githubusercontent.com/proycon/folia/master/setdefinitions/naf\_pos.foliaset.xml} -- Minimal set of part-of-speech tags as used and defined by NAF \cite{NAF}.
\end{itemize}
\end{setdefinitions}
...
\begin{itemize}
    \item
        \url{http://ilk.uvt.nl/folia/sets/frog-mblem-nl} -- An open set for Dutch lemmas, used by \emph{Frog}\footnote{https://languagemachines.github.io/frog}.
\end{itemize}
\end{setdefinitions}
...
\subsection{Subjectivity Annotation}

\status{final}{pynlpl,libfolia}

\begin{notice}
The span annotation element \texttt{sentiment} (See
section~\ref{sec:sentiment}) usually provides a better suited alternative
for subjectivity annotation than this \texttt{subjectivity} element. Consider using it instead.
\end{notice}
...
multiple tokens. For these we introduce a stand-off annotation in separate
...
is specific to a kind of span annotation. Each span annotation is associated
with a set, for which a declaration should be present in the metadata section
of the document.  The annotation layers are generally embedded within the
structure element that also contains all the words that are referenced. Often
this corresponds to the sentence level. Any other higher level is fine too,
though. Layers are always embedded \emph{after} the word tokens or other
structural elements that they refer to.
...
annotation elements of a more complex nature may require or allow so-called \emph{span
...
relation. Span role elements themselves do not carry any classes and can
...
\begin{lstlisting}[language=xml]
<annotations>
 <entity-annotation set="http://url/to/your/set" />
</annotations>
\end{lstlisting}
\end{declaration}
...
\begin{itemize}
    \item
        \url{https://raw.githubusercontent.com/proycon/folia/master/setdefinitions/namedentities.foliaset.xml}
        -- Generic set definition for named entities, defines the classes:
        \emph{per,org,misc,pro,eve,loc} and others.
    \item \url{http://ilk.uvt.nl/folia/sets/frog-ner-nl}
        -- Set definition for named entities as produced by
        Frog\footnote{\url{https://languagemachines.github.io/frog}}, defines the classes: \emph{per,org,misc,pro,eve,loc}
\end{itemize}
\end{setdefinitions}
...
A very typical form of span annotation is syntax annotation, or constituency
parses. This is represented using the \texttt{syntax} layer which introduces a nested
hierarchy of syntactic unit (\texttt{su}) elements. It is recommended for each
syntactic unit to have a unique identifier:
...
Dependency relations are relations between spans of tokens, in most cases equal to
syntactic units. A dependency relation takes a particular class and
...
to which it is related, and show how you can establish links between the two
layers. The \texttt{dependency} element always contains two span roles: one
head element (\texttt{hd}) and one dependent element (\texttt{dep}). The words
they cover are reiterated in the usual fashion, using \texttt{wref}. For a
better understanding, Figure~\ref{fig:depparse} illustrates the syntactic parse
with the dependency relations.
...
\begin{itemize}
    \item
        \url{http://ilk.uvt.nl/folia/sets/frog-depparse-nl}
        -- Set definition for dependency annotation, as used by the
        sofware Frog\footnote{\url{https://languagemachines.github.io/frog}}
        and Alpino\footnote{\url{https://www.let.rug.nl/vannoord/alp/Alpino/}}.
\end{itemize}
\end{setdefinitions}
...
Unlike a full syntactic parse, chunking is a form of shallow parsing that is
not nested. FoLiA uses a separate annotation type for this. The layer is called
\texttt{chunking} and the span annotation element itself is called
\texttt{chunk}.
...
The \texttt{chunk} element may also carry classes, assigning categories in a shallow
parsing tagset, in which case a declaration is mandatory:
...
\begin{itemize}
    \item
        \url{http://ilk.uvt.nl/folia/sets/frog-chunker-nl}
        -- Set definition for chunking, as used by the
        sofware Frog\footnote{\url{https://languagemachines.github.io/frog}}.
\end{itemize}
\end{setdefinitions}
...
\status{since 0.9, added predicates in v1.3}{pynlpl,libfolia}
...
Semantic roles are usually embedded within the \texttt{predicate} span
annotation element (since FoLiA v1.3). This is a \emph{separate} span
annotation element, which itself may also take a class. Such a class can for
instance be used to describe frame semantics, such as
FrameNet\footnote{\url{https://framenet.icsi.berkeley.edu/fndrupal/}}.

Semantic roles without predicates are also allowed, but less expressive as
relations between the semantic roles are not explicit.

The \texttt{hd} span role can optionally be used to mark the head of a semantic role:
...
<annotations>
 ...
 <predicate-annotation set="http://url/to/your/set" />
</annotations>
\end{lstlisting}
\end{declaration}
...
The \texttt{observation element} is a span annotation element that makes an observation
pertaining to one or more word tokens. It is embedded in an
\texttt{observations} layer.
...
The element may for example act as a more generic replacement for the
\texttt{errordetection} element (section~\ref{sec:errordetection}), or to
encapsulate observations from teachers/proofreaders on a text, in which case it
is often used with the \texttt{desc} element (see section~\ref{sec:desc}). The following example shows different
types of observations from two fictitious sets:
...
<s xml:id="example.p.1.s.1">
 ...
 <observations>
  <observation class="encouragement" set="http://somewhere/teacherobservations.set.xml"
   annotator="teacher234" annotatortype="manual">
...
\begin{lstlisting}[language=xml]
<annotations>
 <observation-annotation set="http://url/to/your/set" />
</annotations>
\end{lstlisting}
\end{declaration}
...
expressed in text. The \texttt{sentiment} span annotation element is used to
this end. It is embedded in a \texttt{sentiments} layer. This element offers a
more comprehensive solution replacing the older \texttt{subjectivity}
element.

The \texttt{sentiment} element takes the following span roles:
\begin{itemize}
\item \texttt{hd} -- \textbf{required} -- The head of the sentiment; expresses the actual sentiment, it covers word spans such as ``happy'', ``very satisfied'', ``highly dissappointed''.
\item \texttt{source} -- \textbf{optional} -- The source/holder of the sentiment, assuming it is explicitly expressed in the text.
\item \texttt{target} -- \textbf{optional} -- The target/recipient of the sentiment, assuming it is explicitly expressed in the text.
\end{itemize}
...
The following feature subsets are predefined, whether they are actually used depends on the set, their values (classes) are set-dependent as well:

\begin{itemize}
\item \texttt{polarity}  -- Expresses the whether the sentiment is positive, neutral or negative.
\item \texttt{strength}  -- Expresses the strength or intensity of the sentiment
\end{itemize}

Besides these predefined features, FoLiA's feature mechanism (see Section~\ref{sec:features}) can be used to associate other custom properties with any sentiment.


\begin{declaration}
\begin{lstlisting}[language=xml]
<annotations>
 <sentiment-annotation set="http://url/to/your/set" />
</annotations>
\end{lstlisting}
\end{declaration}
...
\begin{itemize}
    \item
        \url{http://ilk.uvt.nl/folia/sets/frog-mbma-nl} -- Set definition as used by the software Frog.
\end{itemize}
\end{setdefinitions}
...
\begin{notice}
The span annotation element \texttt{observation} (See
section~\ref{sec:observations}) usually provides a better suited alternative
for error detection than this \texttt{errordetection} element. Consider using it instead.
\end{notice}
...
\begin{itemize}
\item ``\texttt{yes}'' -- This is the default and says that the token is followed by a single space.
\item ``\texttt{no}'' -- This states that the token is not followed by a space.
\end{itemize}
...
\subsubsection{Text Class Attribute}
\label{sec:textclassattrib}

\status{final since v1.4.3}{pynlpl,libfolia}

As we have seen, FoLiA allows for multiple text content elements on the same structural elements, these other text
...
to explicitly state which text class was used in establishing the annotation. This is done with the \texttt{textclass}
attribute on any token or span annotation element. By default, this attribute is omitted, which implies it points to the
default \texttt{current} text class.
...
add those as an \emph{alternative} (See Section~\ref{sec:alternatives}), as the usual restrictions apply, there can be
only one of each of a given set.

For span annotation, you can apply the \texttt{textclass} attribute in a similar fashion:
...
\begin{lstlisting}[language=xml]
<annotations>
 <style-annotation set="http://url/to/your/set" />
</annotations>
\end{lstlisting}
\end{declaration}
...
\begin{itemize}
    \item
        \url{https://raw.githubusercontent.com/proycon/folia/master/setdefinitions/style.foliaset.xml}
        -- Generic set definition for styling, defines the classes: \emph{strong, emphasis, literal, highlighted, deleted, inserted, math}
\end{itemize}
\end{setdefinitions}
...
\begin{lstlisting}[language=xml]
<annotations>
 <gap-annotation set="http://url/to/your/set" />
</annotations>
\end{lstlisting}
\end{declaration}
...
errors in text content. Note that the use of \texttt{observation} (see Section~\ref{sec:observations}) is
...
\begin{lstlisting}[language=xml]
<annotations>
 <correction-annotation set="http://path/to/your/set" />
</annotations>
\end{lstlisting}
\end{declaration}
...
\begin{itemize}
    \item
        \url{https://raw.githubusercontent.com/proycon/folia/master/setdefinitions/spellingcorrection.foliaset.xml}
        -- Generic set definition for correction of orthography.
\end{itemize}
\end{setdefinitions}
...
section~\ref{sec:references} do always show in the text.
...
\status{submetadata since v1.5}{pynlpl,libfolia}

FoLiA has support for metadata, most notably the extensive and mandatory declaration section for all used annotations
which you have seen throughout this documentation. To complement this, there is FoLiA's native metadata system, in which
simple metadata fields can be defined and used at will through the \texttt{meta} element. The following example shows
...
Multiple \texttt{foreign-data} blocks are allowed too.

\subsection{Submetadata}
\label{sec:submetadata}

Whereas the metadata discussed in the previous section concerns document-wide metadata, i.e. metadata that is applicable
to the document as a whole, FoLiA also supports metadata on arbitrary parts of the document. This is referred to as
\emph{submetadata}. Within the \texttt{metadata} block, one can include one or more \texttt{submetadata} blocks. Like
\texttt{metadata}, a \texttt{submetadata} block carries a \texttt{type} attribute, a \texttt{src} attribute in case the
metadata is an external reference, and it may hold \texttt{meta} elements, \texttt{foreign-data} elements. It differs
from \texttt{metadata} in that it carries a mandatory \texttt{xml:id} and \emph{never} has an \texttt{annotations} block.
The ID is in turn used to back to the metadata from particular elements in the text. Such a reference is made using the
\texttt{metadata} attribute, which is a common FoLiA attribute allowed on many elements. Consider the following example
...
associated with the existing facilities \texttt{str} or \texttt{t-str} (see Section~\ref{sec:str}. Some redundancy
...
Submetadata is inherited (recursively), i.e. it is not necessary to explicitly assign the \texttt{metadata} attribute to the children
...
\status{Completely renewed since v1.4}{pynlpl}

\section{Introduction}

The sets and classes used by the various linguistic annotation types are never
defined in the FoLiA documents themselves, but externally in \emph{set
definitions}.
...
semantics to the FoLiA documents that use them and are an integral part of
FoLiA.  When set definitions are absent, validation can only be conducted on a
shallow level that is agnostic about all sets and the classes therein.

Recall from Section~\ref{sec:declarations} that all sets that are used need to be
declared in the document header and that they point to URLs holding a FoLiA set
definitions. If no set definition files are associated, then a full in-depth
validation cannot take place.

The role of FoLiA Set Definitions is:

\begin{itemize}
    \item to define which classes are valid in a set
    \item to define which subsets and classes are valid in \emph{features} (see section~\ref{sec:features}) in a set
    \item to constrain which subsets and classes may co-occur in an annotation of the set
    \item to allow enumeration over classes and subsets
    \item to assign human-readable labels to symbolic classes
    \item to relate classes to external resources defining them (data category registries, linked data)
    \item to define a hierarchy/taxonomy of classes
\end{itemize}
...
supported for backwards compatibility. Since FoLiA v1.4 however, we strongly
prefer and recommend to store the set definitions as RDF \cite{RDF}, i.e.\ the
...
\texttt{format} attribute, recognised values are:

\begin{itemize}
	\item \texttt{application/rdf$+$xml} -- XML for RDF (assumed for \texttt{rdf.xml} or \texttt{rdf} extensions
	\item \texttt{text/turtle} -- Turtle\footnote{\url{https://www.w3.org/TeamSubmission/turtle/}} (for RDF) (assumed for \texttt{ttl} extensions)
	\item \texttt{text/n3} -- Notation 3 (for RDF) (assumed for \texttt{n3} extensions)
	\item \texttt{application/foliaset$+$xml} - Legacy FoLiA Set Definition format (XML) (assumed for \texttt{xml} extensions and in most other cases)
\end{itemize}
...
of Simple Knowledge Organization Systems \cite{SKOS}. Not everything can be
expressed in the SKOS model, so we have some extensions to it which are
formally defined in our set definition schema at
\url{https://raw.githubusercontent.com/proycon/folia/master/schemas/foliasetdefinition.ttl}.
The RDF namespace for our extensions is
\texttt{http://folia.science.ru.nl/setdefinition\#}, for which we use the prefix
\texttt{fsd:} generally, though this is mere convention.
...
Turtle syntax is defined by the \texttt{@base} directive at the top of your set
definition. Never reuse the SKOS or FoLiA Set Definition namespaces!
...
of some confusion. We attempt to map the terms in
Table~\ref{tab:skosmapping}.

\begin{table}
\begin{tabular}[h!]{|l|l|l|}
Our term & SKOS & SKOS class \\
\hline
Set/Subset & Collection & \texttt{skos:Collection} \\
Class & Concept & \texttt{skos:Concept} \\
ID & Notation & \texttt{skos:notation} \\
\end{tabular}
\caption{Mapping between our terminology that that of SKOS}
\label{tab:skosmapping}
\end{table}
...
The first two lines state that \texttt{http://your/namespace/\#your-set-id} is
\emph{a}\footnote{the \emph{a} in Turtle is shorthand for rdf:type} SKOS
Collection, which is what we use for FoLiA Sets. The \texttt{skos:notation}
property corresponds to the ID of the Set, only one is allowed\footnote{Technically, SKOS allows multiple, but we restrict it for Set
Definitions}.
...
\texttt{fsd:open} property is used to indicate this, it is not part of SKOS but
...
The \textbf{ID} (\texttt{skos:notation}) of the class is mandatory for FoLiA
Set Definitions and determines a value the \texttt{class} attribute
may take in the FoLiA document, for elements of this set. The
\texttt{skos:prefLabel}
...
following set definition for named entities, in which the \emph{location} class
...
explicitly using the \texttt{skos:narrower} property, as shown in the following excerpt:
...
(\texttt{skos:notation}) of any nested classes
...
for such paths, so the period in the above example (\texttt{loc.country}) is merely a convention. Each
ID, however, does have to be unique in the entire set.

\section{Subsets}

Section~\ref{sec:features} introduced subsets.
These can be defined in a
similar fashion to sets. Consider the legacy XML format first:
...
In RDF, subsets are
defined as SKOS Collections, just like the primary set. The primary set refers
to the subsets using the same \texttt{skos:member} relation as is used for
classes/concepts.
...
(\texttt{\#gender.m} instead of \texttt{\#m}). This is just a recommended
convention as URIs have to be unique and we may want to re-use the \texttt{m}
ID in other subsets as well. The ID in the \texttt{skos:notation} property does not need to
...
\texttt{<feat subset="gender" class="m" />}.
...
The use of \texttt{skos:OrderedCollection} is currently not supported yet,
\texttt{skos:Collection} is mandatory. Ordering of classes (SKOS Concepts) can
currently be indicated through a separate \texttt{fsd:sequenceNumber} property.

FoLiA Set Definitions must be \emph{complete}, that is to say that all sets
(SKOS collections) and classes (SKOS concepts) must be fully defined in one and
the same set definition file\footnote{The file need not be static but can be
dynamically generated server-side}; which must be \emph{publicly} available
from a URL. A set definition must contain one and only one primary set (SKOS
collection), all other sets must be subsets (SKOS collections that are a
member of the primary set, no deeper nesting is supported).
...
Shallow validation can be performed using a RelaxNG schema, the latest version of
which can always be obtained from the following URL: \\
\texttt{https://github.com/proycon/folia/blob/master/schemas/folia.rng}

Using the RelaxNG schema, uou can validate your document using standard XML tools such as
...
It is however recommended to use more dedicated tools such as
\texttt{foliavalidator} (see Appendix D) or \texttt{folialint}, as these use actual FoLiA
libraries can do more validation. %bad! a hard reference! I know, sorry!

The \texttt{foliavalidator} tool can also be used for deep validation.
...
\item \texttt{foliatree} -- Outputs the hierarchy of a FoLiA document.
\item \texttt{foliacat} -- Concatenates two or more FoLiA documents.
\item \texttt{foliamerge} -- Merges the annotations of two or more FoLiA documents into one.
\item \texttt{foliatextcontent} -- A tool for adding or stripping text redundancy, supports adding offset information.
\end{itemize}
