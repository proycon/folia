...
Part-of-Speech annotations may also include extra features, which are explicitly listed and are defined by the set, and a particular subset within that set. The use of subset will be further explained in section~\ref{sec:advparadigm}.
...
For some annotations, associating a single class from a set is not sufficient. FoLiA provides the option to associate multiple classes, or to associate classes with specific subsets. An example of this was seen in the handling of Part-of-Speech tags with feature information.

Take a look at the following abstract case: \texttt{X} is a fictional token/span annotation. It is associated with two classes. The element \texttt{Y} belongs with \texttt{X}, and is custom to the specific annotation.
...
Optionally, \texttt{X} itself can also be associated with a set, as done in the example for part-of-speech tags with features:

\begin{lstlisting}[language=xml]
<w xml:id="example.p.1.s.1.w.2">
    <t>boot</t>
    <pos set="http://ilk.uvt.nl/folia/sets/cgn" class="N">
        <posfeat subset="ntype" class="soort" />
        <posfeat subset="number" class="ev" />
        <posfeat subset="degree" class="basis" />
        <posfeat subset="gender" class="zijd" />
