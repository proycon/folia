...
In substrings, the offset attribute on the text-content element is required. 

Both the \texttt{str} as the \texttt{t} elements are members of the same set and thus share the same classes. The class \emph{current} is assigned when no explicit class is mentioned. In case of multiple \texttt{t} elements the class tells to which \texttt{t} element the substring refer: Both are covered by the \texttt{text-annotation} declaration.
...
\begin{lstlisting}[language=xml]
 <p xml:id="example.p.1">
    ...
    <str xml:id="example.p.1.s.1" class="ocroutput">
    	<t offset="0">Hell0</t>
    </str>
    
    <str xml:id="example.p.1.s.2" class="original">
    	<t offset="0">Hello.</t>
    </str>    
