...
{\bf \small version 0.9.3 -- Revision 2.10} \\ \vspace*{0.5cm}{\huge Documentation}\\
...
<table xml:id="example.table.1">
  ...
  <row>
    ...
    <cell>
      <w xml:id="example.table.1.w.4"><t>Radboud University Nijmegen</t></w>
    </cell>
  </row>
  <row>
    <cell>
...
The \texttt{str} element does not allow all of (extended) token annotations.
...
in some way they modify or mark up textual content. Text markup elements mark certain spans within the text. Elements of this category may be nested.

Text markup elements may carry an optional ID. However, it may happen that textual
content is repeated on multiple levels, which implies the textual markup
elements are also repeated. Only one may carry the customary \texttt{xml:id}
attribute, duplicated may carry the \texttt{id} \emph{reference} attribute (in the FoLiA namespace instead of the XML
...
suited for untokenised text.

Any of the remaining common FoLiA attributes may be freely associated with any
of the text-markup elements.

Each text-markup element starts with \texttt{t-} and demands a declaration, the following subsections will discuss the various text markup elements available.

\subsection{Style}
\label{sec:tstyle}

The text markup element \texttt{t-style} marks a specific portion of textual
concent to be rendered in a specific style. Styles in turn are classes in your
...
Text-markup elements, may always be nested, as the following example shows with
styles:

\begin{lstlisting}[language=xml]
<s>
  <t>to <t-style class="italic"><t-style class="bold">be</t-style></t-style> or not to be</t> 
</s>
\end{lstlisting}
...
possible, but this is not required. The following examples show both ways:
...
\begin{lstlisting}[language=xml]
<s>
  <t>to <t-gap id="gap.1" class="fillin" /> or not to be</t> 
  <w><t>to</t></w>
...
The \texttt{t-str} element relates to the higher annotation element
\texttt{str}, introduced in Section~\ref{sec:str} and it shares the same
declaration. The \texttt{id} reference attribute of \texttt{t-str} may therefore refer to the id of a
\texttt{str} element. The element is use to mark arbitrary substrings:

\begin{lstlisting}[language=xml]
<s>
  <t>to <t-str xml:id="gap.1">be</t-str> or not to be</t> 
</s>
\end{lstlisting}
...
errors in text content:


\begin{lstlisting}[language=xml]
<s>
  <t>to <t-error xml:id="error.1" class="spelling">bee</t-corrected>
  or not to be</t> 
</s>
\end{lstlisting}
...
expressive power. The below example shows both:
...
\item XPath query for the text of all words/tokens:
  ...
  not(ancestor-or-self::*/morpheme) and not(@class)]//text()} \\
