...
{\bf \small version 0.8.1 - revision 2.3} \\ \vspace*{0.5cm}{\huge Documentation}\\
...
\subsection{Meta-corrections}

\status{PROPOSAL}{no}

The element \texttt{metacorrection}, within the annotation layer \texttt{metacorrections}, is a specific kind of alignment that allows you to specify dependency relations between two or more corrections, or their suggestions. Consider the erroneous dutch sentence ``Toen ik naar binnen gingen'', which has a concordancy error and could be either ``Toen ik naar binnen ging'' or ``Toen wij naar binnen gingen'':
...
<s>
  ...
  <metacorrections>
  	<metacorrection class="persoonsvorm_onderwerp_mismatch">
...
The metacorrection has alignment references the correction elements that form a part of it. It can optionally also include \texttt{metasuggestion} elements which in turn contain alignment references the parts that form the suggestions.
...
FoLiA & xhtml & yes, via XSLT \footnote{only support for some token annotations} \\
