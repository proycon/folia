...
More advanced aspects of the FoLiA paradigm will introduced later in section \ref{sec:advparadigm}.
...
The metric element allows annotation of some kind of measurement for the element it is a member of. Example:
...
Tokens can be further segmented into morphemes, a subtoken level of annotation. Morphemes behave quite similarly to \texttt{w} elements (tokens). Moreover, morphemes can be referred to from within in span annotation using \texttt{wref}, allowing spans to be defined not only over whole words/tokens but also parts thereof. The element for morphemes is \texttt{morpheme}, and can only occur within \texttt{w} elements. Recall that \texttt{t} elements can contain references to higher-level \texttt{t} elements. In such cases, the \texttt{offset} attribute is used to designate the offset index in the word's associated text element (\texttt{t}) (zero being right at the start of the text). Morphemes may do this.
...
Note that attribute \texttt{function} is a predefined subclass you may use (not mandatory), its values are defined by the set rather than the FoLiA standard, so they are user-defined.

Morphemes are special as they allow token annotation just as words do. We can for instance bind lemma annotation to the morpheme representing the word's stem rather than only to the entire word:
