...
The above example states that the particular section is final since version 0.4 of FoLiA and that it is implemented in the libraries pynlpl (python) and libfolia ($C++$). You may also see portions of this documentation that are proposals, which means the funcionality is still open for debate and not final yet. Example: 
...
\begin{itemize}
\item \textbf{Structural annotation} - Annotations marking global structure, such as chapters, sections, subsections, figures, list items, paragraphs, sentences, words, morphemes, phonemes etc... Section~\ref{sec:structureannotation} will discuss all structure annotation elements in FoLiA.
...
\begin{itemize}
	...
	\item \textbf{Alignment annotation} - Allows for associations between arbitrary annotations within or accross FoLiA documents.
...
\texttt{Coreferencelink} may take three attributes, which are actually predefined FoLiA subsets (See Section~\ref{features}), their values depend on the set used and are thus customly definable and never predefined: 
...
\status{Proposed in v0.9}{reimplementation necessary}
...
Utterances need not be declared necessarily, but may be if classes are assigned. The actual set in the example is fictituous:
...
The correction of an error implies the detection of an error. In some cases, detection comes without correction, for instance when the generation of correction suggestions is postponed to a later processing stage. The \texttt{errordetection} element is a very simple element that serves this purpose. It signals the existance of errors and is a normal token annotation element: 
...
Section~\ref{features} introduced subsets. These can be defined in a similar fashion to sets and also carry a \texttt{type} attribute:
