...
{\bf \small version 0.9.0 - revision 2.6} \\ \vspace*{0.5cm}{\huge Documentation}\\
...
\item \textbf{Formalised} - The format is formalised, validatable on both a shallow and a deep level (the latter including tagset validation), and easily machine parsable, for which tools are provided.
...
Identifiers are very important and used throughout the FoLiA format, and mandatory for almost all structural elements. They enable external resources and databases to easily point to a specific part of the document or an annotation therein. FoLiA has been set up in such a way that \emph{identifiers should never ever change}. Once an identifier is assigned, it should never change, re-numbering is strictly prohibited unless you intentionally want to create a new resource and break compatibility with the old one. Identifiers can be assigned to many FoLiA elements, their usage is recommended though often not strictly required. 

\section{Basic Structural Elements}
\label{sec:basics}

\subsection{Text Structure Annotation}

Basic structural elements for textual documments occur within the \texttt{text} element. These are the most basic ones:
...
At least the deepest content element should preferably always contain a text element (\texttt{t}) which holds the actual textual content. The necessity of having an explicit text element shall become apparent as you progress through this documentation.
...
FoLiA is not just suited for the annotation of text, but also accomodates annotation of transcribed speech. This generally asks for a different document structure than text documents. The top-level element for speech-centered resources is \texttt{speech}. Certain elements described in the section on text structure may be used under \texttt{speech} as well; such as divisions (\texttt{div}), sentences (\texttt{s)} and words (\texttt{w}). Notions such as paragraphs and figures make less sense in a speech context.

In addition to the existing elements, a new structural element is introduced for speech annotation: 

\begin{itemize}
  \item \texttt{utt} -- \textbf{utterance} -- An utterance may consist out of words or sentences, which in turn may contain words. The opposite is also true, a sentence may consists out of multiple utterances.  
\end{itemize}
 
 
For \emph{all} structure elements in a speech context, the following speech-specific FoLiA attributes are available, on top of the other generic FoLiA attributes:

\begin{itemize}
  \item \texttt{src} -- \textbf{source} -- Points to a file or full URL of a sound or video file. This attribute is inheritable.
   
  \item \texttt{begintime} -- \textbf{begin time} --  A timestamp in \texttt{HH:MM:SS.MMMM} format, indicating the begin time of this speech act. If a sound clip is specified (\texttt{src}); the timestamp refers to an offset in the soundclip (minus \texttt{srcoffset}).
    
  \item \texttt{endtime} -- \textbf{end time} --  A timestamp in \texttt{HH:MM:SS.MMMM} format, indicating the end time of this speech act. If a sound clip is specified (\texttt{src}); the timestamp refers to an offset in the soundclip (minus \texttt{srcoffset}).
  
  \item \texttt{srcoffset} -- \textbf{source offset} -- A timestamp in \texttt{HH:MM:SS.MMMM} format that is \emph{subtracted} from all \texttt{begintime}, and \texttt{endtime} designation within its scope, to find the position in the audio clip. This attribute enabled the use of global timings in all begintime/endtime attributes, across different sound clips. Defaults to zero.
...
The set attribute is mandatory\footnote{Technically, it can be omitted, but then the set defaults to ``undefined''. This is allowed for flexibility and less explicit usage of FoLiA in limited settings, but not recommended!} and refers to a URL of a FoLiA Set Definition file (see chapter \ref{fsd}). In the above example, the set URLs are fictitious, make sure to list your own appropriate sets. A Set Definition specifies exactly what classes are allowed in the set. It for examples specifies exactly what Part-of-Speech tags exist. This information is necessary to completely validate the document at its deepest level. If the sets point to URLs that do not exist or are not URLs at all,' warnings will be issued. Validation can still proceed but with the notable exception of deep validation of these sets.
...
Look at the following example, showing a full FoLiA document with structured divisions. The declared set is a fictituous example:
...
For more fine-grained control over timed events, for example within sentences. It is recommended to use the \texttt{timesegment} span annotation element instead! This works in a very similar fashion but uses an offset annotation layer. See section~\ref{timesegment}.
...
Part-of-Speech annotation allows the annotation of lexical categories using the \texttt{pos} element. The following example illustrates a simple Part-of-Speech annotation for the Dutch word ``boot'':
...
Lexical annotation can take more complex forms than assignment of single part-of-speech tag. There may for example be numerous features associated with the part of speech tag, such as gender, number, case, tense, mood, etc... FoLiA introduces a special paradigm for dealing with such features. We will look into this later, in section~\ref{sec:advparadigm}. 

Whenever Part-of-Speech annotations are used, they should be declared in the \texttt{annotations} block as follows, the set you use may differ and all attributes are optional. In the declaration example here it is as if the annotations were made by the software \emph{Frog}, but you may want to use your own. Do note the requirement of a \texttt{token-annotation} as well.
...
The metric element allows annotation of some kind of measurement with any content element. Example:
...
This element is technically not a token annotation element, it may be used in a wider scope. It can also be used as a kind of second order annotation within any token annotation elements and within any span annotation elements:
...
The correction of an error implies the detection of an error. In some cases, detection comes without correction, for instance when the generation of correction suggestions is postponed to a later processing stage. The \texttt{errordetection} element is a very simple element that serves this purpose. It signals the existance of errors:
...
Not all annotations can be realised as token annotations. Some typically span multiple tokens. For these we introduce a kind of offset notation in separate \emph{annotation layers}. These annotation layers are embedded at the sentence level, \emph{after} the word tokens. Within these layers, references are made to these word tokens using the \texttt{wref} element. Each annotation layer is specific to a kind of span annotation, for which a declaration should be present in the metadata section of the document.
...
Time segments may also be nested. As always, the classes in the example are set-defined rather than predefined by FoLiA. The (optional) features \texttt{begindatetime} and \texttt{enddatetime} can be used express the exact moment at which an event started or ended. Note that this differs from the generic \texttt{datetime} attribute, which would describe the time at which the annotation was recorded, rather than when the event took place! 

The timing layer is ideally suited for segmenting speech and relating it to an annotatable transcription. Precisely for this purpose the timing layer can take an extra attribute \texttt{src}, this attribute specifies the filename or URL of a sound or video file with speech. All begindatetime and enddatetime attributes are then to be interpreted specifically for this file.
...
<s>
 ...
 <w xml:id="example.p.1.s.1.w.6"><t>go</t></w>
 <w xml:id="example.p.1.s.1.w.7"><t>.</t></w>
 <timing src="ithinkihavetogo.mp3">
  <timesegment begindatetime="00:00:00.000" 
   enddatetime="00:00:00.250">
    <wref id="example.p.1.s.1.w.1" t="I" />
  </timesegment>
  <timesegment begindatetime="00:00:00.250" 
   enddatetime="00:00:00.500">
    <wref id="example.p.1.s.1.w.2" t="think" />
  </timesegment>
  <timesegment begindatetime="00:00:00.500" 
   enddatetime="00:00:00.750">
    <wref id="example.p.1.s.1.w.3" t="I" />
  </timesegment>
  <timesegment begindatetime="00:00:00.750" 
   enddatetime="00:00:01.000">
    <wref id="example.p.1.s.1.w.4" t="have" />
  </timesegment>
  <timesegment begindatetime="00:00:01.000" 
   enddatetime="00:00:01.250">
    <wref id="example.p.1.s.1.w.5" t="to" />
  </timesegment>
  <timesegment begindatetime="00:00:01.250" 
   enddatetime="00:00:01.500">
    <wref id="example.p.1.s.1.w.6" t="go" />
  </timesegment>
 </timing>
</s>
...
If you are only interested in a annotation of events, and a more coarser level of annotation suffices, then use the structure annotation element \texttt{event} instead. See section~\ref{events}.
...
\texttt{Coreferencelink} may take three attributes, which are actually predefined FoLiA subsets (See Section~\ref{subset}), their values depend on the set used and are thus customly definable and never predefined: 
...
\subsection{Features, Subsets and multiple classes}
\label{subset}
...
A more thorough example for Part-of-Speech tags with features will be explained in section~\label{sec:posfeat}.
...
Features can also be used to assign multiple classes within the same subset, which is impossible with main classes. In the following example the event is associated with a list of two actors. In this case the XML attribute shortcut no longer suffices, and the \texttt{feat} element must be used.
...
To recap: the \texttt{feat} element can always be used to freely to associate \texttt{any} additional classes of \emph{any} designed subset with with \emph{any} annotation element. For certain elements, there are predefined subsets, in which case you can assign them using the XML attribute shortcut. This however only applies to the pre-defined features.
...
In FoLiA, this attribute \texttt{head} is implicitly associated with the subset ``head'' of whatever set you defined. This would thus be equal to: 
...
\section{Morphological Annotation / Subtoken Annotation}

\status{heavily revised since v0.9}{reimplementation necessary}

Tokens can be further segmented into morphemes, a subtoken level of annotation. Morphemes behave quite similar to \texttt{w} elements (tokens). Moreover, morphemes can be referred to in span annotation, allowing spans to be defined not only over whole words/tokens but also parts thereof. The element for morphemes is \texttt{morpheme}, and can only occur within \texttt{w} elements. Recall that \texttt{t} elements can contain references to higher-level \texttt{t} elements. In such cases, the \texttt{offset} attribute is used to designate the offset index in the word's associated text element (\texttt{t}) (zero being right at the start of the text). Morphemes may do this.

Furthermore, a morpheme may take a class, usually referring to its type. As always, the classes are defined by the declared set, and not predefined by the FoLiA set. Declaration proceeds as follows:
...
An example of morphology in use could look as follows:
...
Note that attribute \texttt{function} is a predefined subclass you may use (not mandatory), its values are defined in the set rather than the FoLiA standard, so are user-defined.

Morphemes are special as they allow token annotation just as words do. We can for instance bind a lemma annotation to the morpheme representing the word's stem rather than only to the entire word:
...
<w xml:id="example.p.4.s.2.w.4">
    ...
    <lemma class="lezen" />
    <morphology>
        <morpheme xml:id="example.p.4.s.2.w.4.m.1" class="stem" function="lexical">
			<lemma class="lezen" />
            <t offset="0">lees</t>
        </morpheme>
        <morpheme xml:id="example.p.4.s.2.w.4.m.2" class="suffix" function="inflexional">
            <t offset="4">t</t>
        </morpheme>
    </morphology>
</w>
...
Similarly, consider the Spanish word or phrase ``Dámelo'' (give it to me), written as one entity. If this has not been split during tokenisation, but left as a single token, you can annotate its morphemes, as all morphemes allow token annotation to be placed within its scope:
...
The next example will illustrate how morphemes can be referred to in span annotation. Here we have a morpheme, and not the entire words, which forms a named entity:
...
Recall from section~\ref{declarations} that all used sets need to be declared in the document header and that they point to URLs holding a FoLiA set definition. If no set definition files are associated, then a full in-depth validation can not take place.
