...
\begin{flushleft}
PI Group Language and Speech Technology PI Group \\
...
corpora. Our aim is to introduce a single rich format that can acconmodate a wide variety of linguistic annotation types through a single generalised paradigm. We do not commit to any label set, language or linguistic theory. This is always left to the developer of the language resource, and provides maximum flexibility.
...
The FoLiA paradigm is visualised in the Figure~\ref{fig:paradigm}. Note that the more advanced aspects of the FoLiA paradigm, the higher-order annotation categories, will be introduced later in Section~\ref{sec:higherorder}.
...
The element \texttt{<br />} may be used as both as a structural element as well
...
is also able to operate with IMDI or CMDI metadata \cite{CMDI}, either in external file or stored inline. Note however that storing CMDI or IMDI inside your FoLiA document may cause problems when you want to validate your FoLiA document. It is also incompatible with CMDI or IMDI editors that are unaware of FoLiA.
...
\item XPath query for the text of all words/tokens:\\
  ...
  not(@class)]//text()} \\
    ...
    and makes sure to select only the text content element in case there are
    multiple. (See also Section~\ref{sec:textcontent}). The
...
which can always be obtain from the following URL: \\ \texttt{https://github.com/proycon/folia/blob/master/schemas/folia.rng}
...
In case your can not find a suitable way to encode your particular annotation
in the latest version of the FoLiA specificaton, we urge you to write to
\texttt{proycon\@anaproy.nl} so that discuss whether FoLiA needs to be expanded
