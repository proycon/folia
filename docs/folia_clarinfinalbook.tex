...
\author{Maarten van Gompel\footnote{Centre for Language and Speech Technology, Radboud University} \and Ko van der Sloot\footnotemark[\value{footnote}] \and Martin Reynaert\footnotemark[\value{footnote}] \footnote{Tilburg Centre for Cognition and Communication, Tilburg University}  \and Antal van den Bosch\footnote{Centre for Language Studies, Radboud University} }
\date{}
...
different annotation types. FoLiA is being develed in a bottom-up and
...
An extensive overview of the FoLiA format is presented in \citeasnoun{FOLIACLIN2013},
...
\citeasnoun{FOLIADOC2014} and offers a reference guide to all elements and attributes
...
University, are also bundled in our LaMachine
...
is part of the larger \textsf{PyNLPl}
...
\textsf{libfolia}\footnote{https://github.com/languagemachines/libfolia}, the
...
\textsf{OpenConvert}\footnote{https://github.com/INL/OpenConvert}, developed by the Institute for Dutch Lexicology (INL), which can convert from TEI, plaintext, Alto, Microsoft Word, and HTML to
FoLiA.
...
web browser. The \texttt{folia2html} conversion tool also employs the same
stylesheet.
...
\textsf{foliaquery}, part of the FoLiA tools. This tool reads one or more FoLiA
...
and operate on these indices. The software \textsf{Blacklab} (back-end)
\cite{BLACKLAB} (front-end) \textsf{Whitelab} \cite{WHITELAB} are examples of this. They were
...
\textsf{PyNLPl} includes a library that converts CQL to the more expressive but
...
Therefore, the \textsf{foliaquery} command-line tool and the FQL library it relies on
...
such an environment, we have been developing the \emph{FoLiA Linguistic Annotation Tool}
...
\begin{itemize}
\item \textbf{\textsf{Cesax}} -- A co-reference editor for syntactically annotated XML corpora.
...
\item \textbf{\textsf{CLAM}}\footnote{https://proycon.github.io/clam} -- Turns command-line NLP tools into RESTful webservice with an
    interface for human end-users. It integrates the FoLiA viewer to visualise
    FoLiA documents. \cite{CLAM}
\item \textbf{\textsf{Ucto}}\footnote{https://languagemachines.github.io/ucto} -- An advanced rule-based tokeniser for a variety of
    languages. Support FoLiA
    input and output. \cite{UCTO}
\item \textbf{\textsf{Frog}}\footnote{https://languagemachines.github.io/frog} -- An NLP suite for Dutch, implementing tokenisation (through
...
\item \textbf{\textsf{Gecco}}\footnote{https://github.com/proycon/gecco}  -- Generic Environment for Context-Aware Correction
    of Orthography: A spelling correction engine fully based on FoLiA. Powers
    \emph{Valkuil.net} and soon also \emph{Fowlt.net}.
\item \textbf{\textsf{Ticcl}} -- Text-Induced Corpus Clean-up. Outputs FoLiA. Used in the
